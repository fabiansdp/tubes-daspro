...
def beli_tiket(Username):
    ...
    if syarat_umur(umur,batasan_umur) and syarat_tinggi(tinggi,batasan_tinggi): #Memenuhi syarat
        if (saldo>(jml*harga)):   #Saldo memenuhi
            ...
            for i in range (panjang(kepemilikandata)):      #Pada username dan id wahana yang sama di kepemilikandata jumlah tiket ditambah
                if (Username==kepemilikandata[i][0] and id==kepemilikandata[i][1]):
                    kepemilikandata[i][2]=str(int(kepemilikandata[i][2])+jml)
                    didata=True
            if didata==False:                               #Jika username dan id wahana tidak ada yang sama di kepemilikandata
